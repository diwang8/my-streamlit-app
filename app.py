    ...
    if param == "最高价格":
        ...
        for price in range(int(current_price) + 20, int(current_price * 2) + 1, 20):
            ...
            if result and result <= target_days:
                suggestions["最高价格"] = f"建议提高至 {price} 元（投资者回本周期：{result} 天）"
                return suggestions
        ...
        for extra_days in range(30, 181, 30):
            ...
            result = simulate(df)
            if result and result <= target_days:
                suggestions["周期"] = f"建议延长至 {df['周期'].iloc[0]} 天（投资者回本周期：{result} 天）"
                return suggestions
        ...
        for val in [0, 1]:
            ...
            result = simulate(df)
            if result and result <= target_days:
                suggestions["是否常驻"] = f"建议设为 {'是' if val == 1 else '否'}（投资者回本周期：{result} 天）"
                return suggestions
        ...
        for val in [0, 1]:
            ...
            result = simulate(df)
            if result and result <= target_days:
                suggestions["剧场规模"] = f"建议使用 {'大中剧场' if val == 1 else '小剧场'}（投资者回本周期：{result} 天）"
                return suggestions
        ...
        for val in region_map.values():
            ...
            result = simulate(df)
            if result and result <= target_days:
                suggestions["剧场区域"] = f"建议调整为 {reverse_region_map[val]}（投资者回本周期：{result} 天）"
                return suggestions
        ...
        for tag, val in tag_values.items():
            ...
            result = simulate(df)
            if result and result <= target_days:
                suggestions[f"题材标签：{tag}"] = f"建议添加该标签（投资者回本周期：{result} 天）"
                return suggestions
